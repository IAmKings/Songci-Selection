...
- AC1 iOS 点击直达:**代码完成,未编译验证**(本机 Xcode 缺 iOS 26.2 platform),待环境就绪后 xcodebuild + 真机
- AC2 macOS 点击直达:✅ 实机验证跳转成功;TCC 弹窗("访问其他App的数据")为开发期重签所致,正式 Developer ID 签名后仅首次授权一次——已接受,不处理
- AC3 无深链启动:✅ desktopTest 21/0 + 实机正常
- AC4 构建产物:macOS ✅ / iOS 待上

### 关键决策记录

- App Group 同步延迟方案**已回退**:delay 放在数据加载链路会阻塞仓库初始化,启动卡 LoadingScreen 5 秒再弹窗,体验更差;保持立即同步,弹窗由正式签名解决

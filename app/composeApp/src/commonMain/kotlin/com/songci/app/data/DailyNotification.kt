...
/**
 * 通知选词:独立随机,复用 randomPoems 查询(异常字符过滤 ⿰/缺字/超长词牌/单行 + 乱码排除,
 * 与首页/小组件同源推荐规范)。每次调用打开驱动(排期/触发低频场景,开销可接受)。
 */
suspend fun pickRandomPoem(): Poem? = withContext(Dispatchers.Default) {
    SongciDb(createDatabaseDriver()).songciDbQueries.randomPoems(1L).executeAsList()
        .firstOrNull { !it.content.containsGarbled() }
        ?.let { Poem(it.id, it.rhythmic, it.content, it.author_id, it.author_name) }
}

/**
 * 通知首句:超长句截断(长调句群单行可达 78 字,撑满系统通知难看)。
 * 截断位置补省略号;maxChars 按字符计。
 */
fun Poem.notificationFirstLine(maxChars: Int = 24): String {
    val line = content.lineSequence().firstOrNull { it.isNotBlank() } ?: return ""
    return if (line.length > maxChars) line.take(maxChars).trimEnd() + "…" else line
}
